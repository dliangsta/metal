'''
Adapted from
https://github.com/wielandbrendel/bag-of-local-features-models/blob/master/bagnets/pytorch.py
'''
...
class Bagnet(Module):
  ...
  def __init__(self, block, layers, strides=[1, 2, 2, 2], kernel3=[0, 0, 0, 0], num_classes=1000, avg_pool=True, **kwargs):
    self.inplanes = 64
...
class Bagnet33(Bagnet):

  def __init__(self, pretrained=True, **kwargs):
    super().__init__(Bottleneck, [3, 4, 6, 3], strides=[2, 2, 2, 1], kernel3=[1,1,1,1], **kwargs)
    if pretrained:
      self.load_state_dict(model_zoo.load_url(model_urls['bagnet33']))

class Bagnet17(Bagnet):

  def __init__(self, pretrained=True, **kwargs):
    super().__init__(Bottleneck, [3, 4, 6, 3], strides=[2, 2, 2, 1], kernel3=[1,1,1,0], **kwargs)
    if pretrained:
      self.load_state_dict(model_zoo.load_url(model_urls['bagnet17']))

class Bagnet9(Bagnet):
  """Constructs a Bagnet-9 model.
  Args:
    pretrained (bool): If True, returns a model pre-trained on ImageNet
  """
  def __init__(self, pretrained=True, strides=[2, 2, 2, 1], **kwargs):
